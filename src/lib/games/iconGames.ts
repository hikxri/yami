...
import type { IconGameConfig, IconGameSettings } from "./types";
import { log, logError } from "../log";

export async function runIconGame(
  interaction: ChatInputCommandInteraction,
  gameName: string,
  config: IconGameConfig,
  settings: IconGameSettings,
): Promise<void> {
  ...
  let data;
  try {
    data = await config.getAnswer(settings);
  } catch (e) {
    logError(e);
  ...
  const { gameImage, originalImage, name } = data;
  ...
  collector.on("collect", async (message: Message) => {
    try {
      ...
      async function replyWithHint() {
        hintCount++;
        const hint = config.getHint(name, hintCount);
